...
public class WebClient {
  ...
  private static final String HTML_SEARCH_URL = "https://html.duckduckgo.com/html/search?q=";
  private static final String API_SEARCH_URL = "https://api.duckduckgo.com/";

  private final HttpClient httpClient;

  public WebClient(Duration timeout, boolean logRequests, boolean logResponses) {
    ...
    return RetryUtils.retryPolicyBuilder()
        ...
        .withRetry(() -> {
          ...
          if (results.isEmpty()) {
            ...
            return List.of(WebSearchResult.builder()
  ...
  CompletableFuture<List<WebSearchResult>> searchAsync(WebSearchRequest request) {
  ...
  private String buildUrl(WebSearchRequest request) {
    ...
    if (!StringUtil.isNullOrEmpty(searchTerms) && searchTerms.trim().startsWith("http")) {
      return searchTerms.trim();
    } else {
      return HTML_SEARCH_URL + request.searchTerms();
    }
  }

  private List<WebSearchResult> performHtmlSearch(WebSearchRequest request) throws IOException {
    ...
    List<WebSearchResult> results = parseHtmlResults(doc, limit);
  ...
  private List<WebSearchResult> performApiSearch(WebSearchRequest request) {
    String url =
        API_SEARCH_URL + "?q=" + Utils.urlEncode(request.searchTerms())
  ...
  private List<WebSearchResult> parseHtmlResults(Document doc, int maxResults) {
      ...
      if (snippet.isEmpty()) {
        ...
        for (Element link : links) {
          ...
          if (!linkText.isEmpty() && !linkText.equals(title)) {
            if (!bodyFromLinks.isEmpty())
              bodyFromLinks.append(" ");
  ...
  private void addField(JsonNode root, List<WebSearchResult> results, String field, int limit) {
  ...
  private List<WebSearchResult> parseApiResponse(String json, Integer maxResults) {
    List<WebSearchResult> results = new ArrayList<>();
  ...
  private boolean isValidUrl(String url) {
    if (url == null || url.isEmpty())
      return false;
    if (url.contains("duckduckgo.com") && !url.contains("/l/"))
      return false;
  ...
  private void addRelatedTopicResult(JsonNode topic, List<WebSearchResult> results) {
